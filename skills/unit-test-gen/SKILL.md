...
name: unit-test-gen
description: >
  仓库级单元测试自动生成。自动扫描源码、解析函数签名与行为、按四个维度
  （功能性/边界/异常容错/数据完整性）动态生成单元测试用例、执行并输出报告。
...
全自动的仓库级单元测试生成技能。扫描代码 → 分析函数行为 → 按四个维度生成
测试 → 执行并出报告。全程无需人工介入。
...
| 检测项 | 检测方式 |
|--------|---------|
| 语言 | 文件扩展名（`.py` → python，`.cpp`/`.h` → cpp） |
| 框架 | 项目配置文件（如 `pyproject.toml`、`CMakeLists.txt`） |
| 混合仓库 | 按扩展名分组，支持多语言并存，单语言优先 |

检测结果写入 `testcases.json` 的 `languages` 和 `test_frameworks` 字段。
各语言的具体测试框架和生成规则见 `references/` 下的语言参考文档。
...
7. **写入 `testcases.json`**：只存元数据（签名、MD5、维度、测试 case 描述），不存测试代码。
...
3. **为每个文件生成测试**：
   - 计算输出路径（镜像源码目录结构）。
   - 按 `testcases.json` 中的 case 描述，生成对应的测试函数。
